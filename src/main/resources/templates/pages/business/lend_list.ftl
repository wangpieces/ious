...
<div class="my_content" style="border:0px solid red;margin-top:0.2rem;">
    <div style="width:7.5rem;border-bottom:1px solid #eff0f1;height:1.5rem;">
       <ul class="my_content_title">
           <li class="my_content_title_active" style="">待收总额<p>${iousMoney.allMoney} 元</p></li>
           <li>7天待收<p>${iousMoney.day7Money} 元</p></li>
           <li>30天待收<p>${iousMoney.day30Money} 元</p></li>
       </ul>
    </div>
    <div class="search">
        <input type="text" name="search" id="search" placeholder="输入借款人姓名" onblur="search(this.value);"/>
...
$(function(){
    ...
    $('.my_content_title li').click(function(){
        ...
        if($(this).index() == 0){//待收总额
            $('#loadTimeType').val(1)
            getIousList(4,1,keyWord);
        }else if($(this).index() == 1){//7天待收
             $('#loadTimeType').val(2)
             getIousList(4,2,keyWord);
        }else if($(this).index() == 2){//30天待收
            $('#loadTimeType').val(3)
            getIousList(4,3,keyWord);
        }
    });
    getIousList(4,1,'');
});
...
function gotoDetail(id) {
    //跳转到详细页面
    location.href='${iousContextPath}/business/lendListItem?iousId='+id+'&type=2';
}
//关键字搜索
function search(keyWord){
    var loadTimeType = $('#loadTimeType').val();
    getIousList(4,loadTimeType,keyWord);
}
